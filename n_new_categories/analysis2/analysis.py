...
class Gamma_minimax_Problem(object):
    ...
    # def calc_Risks_tensor(self, n_sample=30, estimator = None, distributions = None):
    #     '''calculate Risks of distributions and estimator via Monte Carlo. when estimator = None, use the current estimator in the object. when distributions = None, use the distributions in the object'''
    ...
    #     samples = self.draw_sample(n_sample=n_sample, distributions=distributions)
        
    #     estimates = estimator(samples.reshape(n_distr*n_sample, -1)).view(n_distr, n_sample, -1)
    #     Risks = self.Risk_fun(estimates, true_parameters)
    #     if distributions is None:
    #         self.Risks = Risks
    #     return Risks
...
b_ub = np.array([-.95, 55., -40.])
prior_credible_range = torch.tensor([30., 65.])
...
OrlitskySureshWu_est = OrlitskySureshWu_Estimator(sample_size, new_sample_size)(data)
ShenChaoLin_est = ShenChaoLin_Estimator(sample_size, new_sample_size)(data)
