...
def SGDmax(estimator, Prior_Risk_Constraint_object, n_iter=3000,\
...
def calc_Gamma_minimax_estimator(estimator, Prior_Risk_Constraint_object, b_ub=None, b_eq=None,\
    ...
    n_SGDmax_iter: number of iterations in SGDmax (use 30 X n_SGDmax_iter for the initial grid)
    ...
    lower, upper = SGDmax(estimator, Prior_Risk_Constraint_object, n_iter=n_SGDmax_iter * 30, use_init_prior=False, optimizer=optimizer, n_sample=n_SGDmax_sample, b_ub=b_ub, b_eq=b_eq)
    ...
    for l in range(max_enlarge_iter):
        ...
        with torch.no_grad():
            Risks = Prior_Risk_Constraint_object.calc_Risks_tensor_memeff(estimator, n_sample=n_accurate_Risk_sample)
        np_A_ub, np_A_eq = Prior_Risk_Constraint_object.get_constraint_matrices()
        linear_prog_result = linprog(-Risks.clone().detach().cpu().numpy(), A_ub=np_A_ub, b_ub=b_ub, A_eq=np_A_eq, b_eq=b_eq, bounds=(0., 1.), method='revised simplex', x0=Prior_Risk_Constraint_object.prior_prob.cpu().numpy())
        if not linear_prog_result.success:
            linear_prog_result = linprog(-Risks.clone().detach().cpu().numpy(), A_ub=np_A_ub, b_ub=b_ub, A_eq=np_A_eq, b_eq=b_eq, bounds=(0., 1.), method='revised simplex')
...
class nnet_estimator(torch.nn.Module):
    def __init__(self, n_transform_hidden_nodes=10, n_transformed_nodes=5, n_aggregated_hidden_nodes=10, activation=torch.nn.functional.relu):
        ...
        self.aggregate_hidden = torch.nn.Linear(self.n_transformed_nodes, self.n_aggregated_hidden_nodes, bias=True)
        self.aggregate_output = torch.nn.Linear(self.n_aggregated_hidden_nodes, 1, bias=True)
    ...
    def forward(self, input):
        ...
        aggregate_hidden = self.activation(self.aggregate_hidden(aggregated))
        return self.aggregate_output(aggregate_hidden)
